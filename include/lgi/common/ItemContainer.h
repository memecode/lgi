...
public:
	...
	/// Shows a editable label above the item allowing the user to change the
	/// value associated with the column 'Col'
	...
	void SetEditLabelSelection(	int SelStart,
								int SelEnd); // call before 'EditLabel'
...
public:
	...
	bool InDrag();
	LRect GetPos();
	void SetPos(const LRect &r);
	bool Visible() const { return Display() != LCss::DispNone; }
...
template <typename T> int LCastComparison(T val) {
	if (val < 0)
		return -1;
	return val > 0 ? 1 : 0;
}
...
public:
	struct SortParam
	{
		...
		operator bool() const { return Col != INVALID; }
...
public:
	...
	virtual void SetSortingMark(SortParam sort) { sortMark = sort; }
	...
	virtual bool SetSort(SortParam sort, bool reorderItems = true,
						bool setMark = true)
	{
		sortParam = sort;
...
class LgiClass LItemContainer : public LLayout,
...
public:
	...
	/* This defines the default handling of dragging an item.
		ITEM_DRAG_USER - Means the application has to define what
											happens when the user drags and item.
		Ie there is no default. ITEM_DRAG_REORDER - The drag handler will allow the
		user to reorder the items in the container. ITEM_DRAG_OVER - The drag
		handler allows the user to drop the item onto existing items. This is more
		for tree controls where the item is added as a child of the target item.
		ITEM_DEPTH_CHANGE - Allow changes of depth in the tree control.
											If this is on items can change their
		parent during a reorder drop operation.
	...
	*/
	...
	struct ContainerItemsDrag
	{
		...
		{
			return sizeof(ContainerItemsDrag) +
					(items > 1 ? sizeof(LItem *) * (items - 1) : 0);
		}
	...
	struct ContainerItemDrop
	{
		...
		// 1) Is the pointer is roughly between two other items, `prev` and `next`
		// will be set. The `ITEM_DRAG_REORDER` flag needs to be set to see this
		// case.
		LItem *prev = NULL, *next = NULL;

		// 2) If the point is over an item, and not near the edge, then `over` will
		// be set. The `ITEM_DRAG_OVER` flag needs to be set to see this case.
		LItem *over = NULL;
		...
		return prev == i.prev && next == i.next && pos == i.pos;
		}
		bool operator!=(const ContainerItemDrop &i) { return !(*this == i); }
	};
...
protected:
	...
	/// \sa SetDragItem
	virtual bool OnReorderDrop(ContainerItemDrop &dest,
								ContainerItemsDrag &source)
	{
		return false;
	}
	...
	/// \sa SetDragItem
	virtual ContainerItemDrop GetItemReorderPos(LPoint pt)
	{
		return ContainerItemDrop();
	}
...
public:                                                                        \
	...
	LItemColumn *ColumnAt(unsigned i) {
		return i < Columns.Length() ? Columns[i] : NULL;
	}
	...
	/// Resizes all the columns to their content, allowing a little extra space
	/// for visual effect
	...
	/// Sets the drag item flags. This enables the user to organize the items in
	/// the container via drag and drop. Either just reordering or also changing
	/// the heirarchy of the items in a tree control.
	///
	/// The format for the drag and drop is `ContainerItemsFormat` and the data in
	/// that format is described by `ContainerItemsDrag`. Finally when the user
	/// drops the items the control handles the drop via `OnReorderDrop`.
