...
int AppWnd::OnCommand(int Cmd, int Event, OsView Handle)
{
	switch (Cmd)
	{
		...
		case IDM_NEW_DLG:
		{
			NewObject(0, TYPE_DIALOG);
			break;
		}
		case IDM_NEW_STRGRP:
		{
			NewObject(0, TYPE_STRING);
			break;
		}
		case IDM_NEW_MENU:
		{
			NewObject(0, TYPE_MENU);
			break;
		}
		case IDM_CLOSE:
		{
...
Resource *AppWnd::NewObject(GXmlTag *load, int Type, bool Select)
{
	Resource *r = 0;
	ObjTreeItem *Dir = 0;
...
void AppWnd::ImportLang()
{
	...
	if (Select.Open())
	{
		...
		if (F.Open(Select.Name(), O_READ))
		{
			ResFileFormat Format = GetFormat(Select.Name());
...
char *HeaderStr = "// This file generated by LgiRes\r\n\r\n";
...
public:
	...
	void DefineSymbol(char *Name, char *Value = 0)
	{
...
bool AppWnd::LoadWin32(char *FileName)
{
	bool Status = false;
	GFileSelect Select;
	GHashTable CtrlNames;
	CtrlNames.Add("LTEXT");
	CtrlNames.Add("EDITTEXT");
	CtrlNames.Add("COMBOBOX");
	CtrlNames.Add("SCROLLBAR");
	CtrlNames.Add("GROUPBOX");
	CtrlNames.Add("PUSHBUTTON");
	CtrlNames.Add("DEFPUSHBUTTON");
	CtrlNames.Add("CONTROL");
	CtrlNames.Add("ICON");
	CtrlNames.Add("LISTBOX");
...
void TestFunc()
{
	const GAutoPtr<GVariant> p(new GVariant);
	// int s = sizeof (p);
	// int as=0;
}
