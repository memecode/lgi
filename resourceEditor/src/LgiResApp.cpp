				...
				for (int n=0; n<b->Length(); n++, Row++)
				{
					...
					switch (c->Type)
					{
						...
						case DATA_BOOL:
						{
							...
							Cell->Add(new LCheckBox(c->Id, c->Label));
							break;
						}
						default:
							LAssert(!"Impl me.");
...
void AppWnd::FindStrings(List<ResString> &Strs, const char *Define, int *CtrlId)
{
	if (Objs)
	{
		List<Resource> l;
...
bool AppWnd::WriteDefines(LStream &Defs)
{
	...
	LHashTbl<ConstStrKey<char>,int> Def;
	LHashTbl<StrKey<char>,const char*> Ident;
...
public:
	...
	ImportDefine *GetDefine(const char *Name)
	{
		if (Name)
		{
			for (auto i: *this)
											...
											if (i)
											{
												...
												else
												{
													if (!MenuNewLang)
													{
														...
														{
															auto id = Defines.GetDefine(i->GetStr()->GetDefine());
															if (id)
															{
