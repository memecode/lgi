...
bool LMenuItem::Visible() { return true; }
bool LMenuItem::Focus() { return (Info.fState & MFS_HILITE) != 0; }
...
int LMenu::_OnEvent(GMessage *Msg)
{
	switch (Msg->Msg())
	{
		case WM_MENUCHAR:
		{
			...
			{
				LMenuItem *Item = (LMenuItem*)Info.dwItemData;
				if (Item)
				{
					LSubMenu *Menu = Item->Parent;
					if (Menu)
					{
						int Index=0;
