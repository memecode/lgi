...
struct KeyConf
{
	...
	unsigned shiftlockmask;
	unsigned numlockmask;
	unsigned capslockmask;
};

/** Return the keysym from keycode.
 */
xcb_keysym_t
key_getkeysym(KeyConf *conf, xcb_keycode_t detail, uint16_t state)
{
    xcb_keysym_t k0, k1;
    ...
    {
		//printf("%s:%i - 2nd group\n", _FL);
        k0 = xcb_key_symbols_get_keysym(conf->keysyms, detail, 2);
        k1 = xcb_key_symbols_get_keysym(conf->keysyms, detail, 3);
    }
    else
    {
		//printf("%s:%i - 1st group\n", _FL);
        k0 = xcb_key_symbols_get_keysym(conf->keysyms, detail, 0);
        k1 = xcb_key_symbols_get_keysym(conf->keysyms, detail, 1);
    }

    /* The numlock modifier is on and the second KeySym is a keypad
     * KeySym */
    if ((state & conf->numlockmask) && xcb_is_keypad_key(k1))
    {
		//printf("%s:%i - numlock\n", _FL);
        ...
        if((state & XCB_MOD_MASK_SHIFT) ||
           (state & XCB_MOD_MASK_LOCK && (state & conf->shiftlockmask)))
            return k0;
        else
            return k1;
    }
			...
			)
	{
		/* The second Keysym is used but if that KeySym is lowercase
			* alphabetic, then the corresponding uppercase KeySym is used
			* instead */
		//printf("%s:%i - shift on, capslock on\n", _FL);
		return k1;
	}
     ...
     * interpreted as ShiftLock, or both */
				...
				(
					(state & XCB_MOD_MASK_LOCK)
					&&
					(state & conf->shiftlockmask)
				)
			)
	{
		//printf("%s:%i - shift lock (k0=%i, k1=%i)\n", _FL, k0, k1);
		return k1 ? k1 : k0;
	}
