			...
			case 24:
			{
				switch (DstBits)
				{
					case 8:
					{
						...
						{
							printf("%s:%i - Error: CBit 24->8 with no palette.\n");
							return 0;
						}
						break;
					}
...
public:
	...
	#endif
	{
		...
		// Get mode stuff
		ScrX = XcbScreen()->width_in_pixels;
		ScrY = XcbScreen()->height_in_pixels;
		ScrBits = XcbScreen()->root_depth;
		
		printf("Screen: %i x %i @ %i bpp (White=%x, Black=%x)\n", ScrX, ScrY, ScrBits, XcbScreen()->white_pixel, XcbScreen()->black_pixel);
