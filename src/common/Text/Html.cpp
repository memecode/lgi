...
public:
	...
	GFlowRegion(GHtml *html, LRect r, bool inbody)
	{
		Html = html;
		MAX.x = cx = x1 = r.x1;
		MAX.y = y1 = y2 = r.y1;
...
bool GHtml::OnFind(LFindReplaceCommon *Params)
{
	bool Status = false;
...
void GHtml::DoFind(std::function<void(bool)> Callback)
{
...
LCursor GHtml::GetCursor(int x, int y)
{
	int Offset = ScrollY();
	ssize_t Index = -1;
	LPoint LocalCoords;
