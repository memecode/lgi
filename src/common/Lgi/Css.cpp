...
int LCss::Len::ToPx(int Box, LFont *Font, int Dpi)
{
	switch (Type)
	{
		...
		case LenPt:
		{
			int EffectiveDpi = Dpi > 0 ? Dpi : LScreenDpi().x;
...
bool LCss::Len::Parse(const char *&s, PropType Prop, ParsingStyle ParseType)
{
	...
	else if (ParseWord(s, "min-content")) Type = LenMinContent;
	else if (ParseWord(s, "max-context")) Type = LenMaxContent;
...
bool LCss::ColorDef::Parse(const char *&s)
{
	...
	NamedColour(ThreeDDarkShadow,  LColour(L_SHADOW).c32())
	NamedColour(ThreeDShadow,      LColour(L_LOW).c32())
	NamedColour(ThreeDFace,        LColour(L_MED).c32())
	NamedColour(ThreeDLightShadow, LColour(L_LIGHT).c32())
	NamedColour(ThreeDHighlight,   LColour(L_HIGH).c32())

	NamedColour(WindowText,        LColour(L_TEXT).c32())
	NamedColour(InfoBackground,    LColour(L_TOOL_TIP).c32())
	NamedColour(Window,            LColour(L_DIALOG).c32())
	NamedColour(AppWorkspace,      LColour(L_WORKSPACE).c32())
	NamedColour(HighlightText,     LColour(L_FOCUS_SEL_FORE).c32())
	NamedColour(Highlight,         LColour(L_FOCUS_SEL_BACK).c32())
