...
LString LSsh::FmtError(LString context)
{
...
LCancel *LSsh::GetCancel() const
{
	return CancelObj;
}

void LSsh::SetLog(LTextLog *log)
{
	Log = log;
}

int LSsh::GetTimeout() const
{
	return timeoutMs;
}

void LSsh::SetTimeout(int ms)
{
	timeoutMs = ms;
}

LSsh::TransferType LSsh::GetTransferType()
{
	return transferType;
}

void LSsh::SetTransferType(TransferType t)
{
	transferType = t;
}

LSsh::ConfigHost LSsh::ConfigHostLookup(const char *Host)
{
	ConfigHost c;
...
void LSsh::SetOverideUnknownHost(bool b)
{
	OverideUnknownHost = b;
}

bool LSsh::Open(const char *Host, const char *Username, const char *Password, bool PublicKey, int Port)
{
	if (sLog)
		sLog->Clear();		
...
LError LSsh::DownloadFile(const char *To, const char *From)
{
	LFile::Path to(To);
...
LError LSsh::DownloadFile(LStream *To, const char *From)
{
	if (!To || !From)
...
LError LSsh::UploadFile(const char *To, const char *From)
{
	if (!To || !From)
		return LError(LErrorInvalidParam);
...
LError LSsh::UploadFile(const char *To, LStream *From)
{
	if (!To || !From)
		return LError(LErrorInvalidParam);
...
LAutoPtr<LSsh::SshConsole> LSsh::CreateConsole(bool createShell)
{
	return LAutoPtr<SshConsole>(new SshConsole(this, createShell));
}

LString LSsh::GetPrompt()
{
	return prompt;
}

void LSsh::SetPrompt(const char *p)
{
	if (p)
	{
		prompt = p;
...
bool LSsh::AtPrompt(LStringPipe &p)
{
	bool found = false;
	ssize_t promptChar = -1;
...
LString LSsh::ReadToPrompt(LSsh::SshConsole *c, LStream *output, LCancel *cancel)
{
	LStringPipe p(1024);
	if (c)
	{
...
bool LSsh::RunCommands(LStream *Console, const char **Cmds, const char *PromptList)
{
	LString Buf;
	int CmdIdx = 0;
	size_t logged = 0;
...
LStructuredLog *LSsh::GetStructLog() const
{
	return sLog.Get();
}

void LSsh::SetStructLog(LAutoPtr<LStructuredLog> slog)
{
	sLog = slog;
}
...
LSsh::SshConsole::SshConsole(LSsh *ssh, bool createShell)
{
	s = ssh;
	Create(createShell);
}

LSsh::SshConsole::~SshConsole()
{
...
bool LSsh::SshConsole::CallMethod(const char *MethodName, LScriptArguments &Args)
{
...
bool LSsh::SshConsole::Create(bool createShell)
{
	LAssert(!channel);
...
ssize_t LSsh::SshConsole::Read(void *Ptr, ssize_t Size, int Flags)
{
	if (channel)
...
ssize_t LSsh::SshConsole::Write(const void *Ptr, ssize_t Size, int Flags)
{
	if (channel)
...
bool LSsh::SshConsole::Write(const LString &s)
{
	if (channel)
...
bool LSsh::SshConsole::Command(LString cmd, LStream *out)
{
	if (!out || !cmd)
		return false;
...
LString LSsh::SshConsole::Command(LString cmd)
{
	LStringPipe p;
	Command(cmd, &p);			
	return p.NewLStr();
...
///////////////////////////////////////////////////////////////////
// IoProgress
LSsh::IoProgress::IoProgress(LSsh *ssh)
{
	s = ssh;
	Start = LCurrentTime();
	Pos = Length = 0;
...
void LSsh::IoProgress::SetLength(uint64_t len)
{
	Length = len;
	if (s->Prog)
	{
...
void LSsh::IoProgress::Value(uint64_t i)
{
	Pos = i;
	if (s->TxtLabel)
	{
