...
void CreateMimeBoundary(char *Buf, int BufLen)
{
	...
	{
		static int Count = 1;
		sprintf_s(Buf, BufLen, "--%x-%x-%x--", (int)LCurrentTime(), (int)(uint64)LCurrentThreadHnd(), Count++);
	}
}
...
char *LMime::GetEncoding()
{
	return Get("Content-Transfer-Encoding");
}

char *LMime::GetCharset()
{
	return GetSub("Content-Type", "Charset");
}

char *LMime::GetBoundary()
{
...
LString LMime::LNewValue(char *&s, bool Alloc)
{
	LString Status;
...
LString LMime::LGet(const char *Field, bool Short, const char *Default)
{
	LString Status;

	if (Field && Headers)
	{
		auto s = StartOfField(Headers, Field);
		if (s)
		{
			s = strchr(s, ':');
			if (s)
...
LString LMime::LGetSub(const char *Field, const char *Sub)
{
	if (!Field || !Sub)
		return LString();

	auto v = LGet(Field, false);
	if (!v)
		return v;
			...
			Header[0] == MimeMagic)
		{
				...
				Source->Read(Mime->Headers, Header[1]) == Header[1])
			{
				...
				if (Source->SetPos(Source->GetPos() + Header[2]) > 0)
				{
					...
					{
						LMime *c = new LMime(Mime->GetTmpPath());
						if (c &&
