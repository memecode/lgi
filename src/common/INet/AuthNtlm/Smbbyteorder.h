...
#define RW_PCVAL(read,inbuf,outbuf,len) { if (read) { PCVAL (inbuf,0,outbuf,len); } else { PSCVAL(inbuf,0,outbuf,len); } }
#define RW_PIVAL(read,big_endian,inbuf,outbuf,len) { if (read) { if (big_endian) { RPIVAL(inbuf,0,outbuf,len); } else { PIVAL(inbuf,0,outbuf,len); } } else { if (big_endian) { RPSIVAL(inbuf,0,outbuf,len); } else { PSIVAL(inbuf,0,outbuf,len); } } }
#define RW_PSVAL(read,big_endian,inbuf,outbuf,len) { if (read) { if (big_endian) { RPSVAL(inbuf,0,outbuf,len); } else { PSVAL(inbuf,0,outbuf,len); } } else { if (big_endian) { RPSSVAL(inbuf,0,outbuf,len); } else { PSSVAL(inbuf,0,outbuf,len); } } }
#define RW_CVAL(read, inbuf, outbuf, offset) { if (read) { (outbuf) = CVAL (inbuf,offset); } else { SCVAL(inbuf,offset,outbuf); } }
#define RW_IVAL(read, big_endian, inbuf, outbuf, offset) { if (read) { (outbuf) = ((big_endian) ? RIVAL(inbuf,offset) : IVAL (inbuf,offset)); } else { if (big_endian) { RSIVAL(inbuf,offset,outbuf); } else { SIVAL(inbuf,offset,outbuf); } } }
#define RW_SVAL(read, big_endian, inbuf, outbuf, offset) { if (read) { (outbuf) = ((big_endian) ? RSVAL(inbuf,offset) : SVAL (inbuf,offset)); } else { if (big_endian) { RSSVAL(inbuf,offset,outbuf); } else { SSVAL(inbuf,offset,outbuf); } } }
...
#define DBG_RW_PCVAL(charmode,string,depth,base,read,inbuf,outbuf,len) { RW_PCVAL(read,inbuf,outbuf,len) DEBUG(5,("%s%04x %s: ", tab_depth(depth), base,string)); if (charmode) print_asc(5, (unsigned char*)(outbuf), (len)); else { int idx; for (idx = 0; idx < len; idx++) { DEBUG(5,("%02x ", (outbuf)[idx])); } } DEBUG(5,("\n")); } 
...
#define DBG_RW_SVAL(string,depth,base,read,big_endian,inbuf,outbuf) { RW_SVAL(read,big_endian,inbuf,outbuf,0) DEBUG(5,("%s%04x %s: %04x\n", tab_depth(depth), base, string, outbuf)); }
#define DBG_RW_IVAL(string,depth,base,read,big_endian,inbuf,outbuf) { RW_IVAL(read,big_endian,inbuf,outbuf,0) DEBUG(5,("%s%04x %s: %08x\n", tab_depth(depth), base, string, outbuf)); }
