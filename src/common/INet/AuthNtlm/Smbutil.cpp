...
#define AddBytes(ptr, header, buf, count) { if (buf && count) { SSVAL(&ptr->header.len,0,count); SSVAL(&ptr->header.maxlen,0,count); SIVAL(&ptr->header.offset,0,((ptr->buffer - ((uint8*)ptr)) + ptr->bufIndex)); memcpy(ptr->buffer+ptr->bufIndex, buf, count); ptr->bufIndex += count; } else { ptr->header.len = ptr->header.maxlen = 0; SIVAL(&ptr->header.offset,0,ptr->bufIndex); } }
#define AddString(ptr, header, string) { char *p = string; int len = 0; if (p) len = strlen(p); AddBytes(ptr, header, ((unsigned char*)p), len); }
#define AddUnicodeString(ptr, header, string) { char *p = string; unsigned char *b = NULL; int len = 0; if (p) { len = strlen(p); b = strToUnicode(p); } AddBytes(ptr, header, b, len*2); }
