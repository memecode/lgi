...
void LProgressPane::UpdateUI()
{
	...
	if (ValText)
		ValText->SendNotify(LNotifyTableLayoutRefresh);
}
...
int LProgressPane::OnNotify(LViewI *Ctrl, const LNotification &n)
{
	switch (Ctrl->GetId())
	{
		case IDC_TABLE:
		{
			if (n.Type == LNotifyTableLayoutChanged)
			{
					...
					tbl_req.Y() > tbl_pos.Y())
				{
					...
					SendNotify(LNotifyTableLayoutChanged);
				}
			}
			break;
		}
...
void LProgressPane::SetDescription(const char *d)
{
	...
	{
		Desc->Name(d);
		Desc->SendNotify(LNotifyTableLayoutRefresh);
	}
}
...
int LProgressDlg::OnNotify(LViewI *Ctrl, const LNotification &n)
{
...
LMessage::Result LProgressDlg::OnEvent(LMessage *Msg)
{
	switch (Msg->Msg())
	{
		#ifdef WIN32
