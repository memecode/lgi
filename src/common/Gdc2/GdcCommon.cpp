...
void LgiWideBorder(GSurface *pDC, LRect &r, LgiEdge Type)
{
	...
	switch (Type)
	{
		...
		case EdgeWin7FocusSunken:
		{
			...
			{
				TopLeft = Med.Mix(Mixer, 0.4f);
				RightBottom = Ws.Mix(Mixer, 0.2f);
			}
			...
			// Edges
			pDC->Colour(TopLeft);
			pDC->Line(r.x1, r.y1+1, r.x1, r.y2-1); // left
			pDC->Line(r.x1+1, r.y1, r.x2-1, r.y1); // top

			// Bottom edge
			pDC->Colour(RightBottom);
			pDC->Line(r.x1+1, r.y2, r.x2-1, r.y2); // bottom
			pDC->Line(r.x2, r.y1+1, r.x2, r.y2-1); // right

			// Inner workspace rect
			pDC->Colour(Ws);
			pDC->Line(r.x1+2, r.y1+1, r.x2-2, r.y1+1); // top
			pDC->Line(r.x1+1, r.y1+2, r.x1+1, r.y2-2); // left
			pDC->Line(r.x2-1, r.y1+2, r.x2-1, r.y2-2); // right
			pDC->Line(r.x1+2, r.y2-1, r.x2-1, r.y2-1); // bottom
			break;
		}
		default:
		{
