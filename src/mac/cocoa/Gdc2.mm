...
void LPalette::Set(int Idx, int r, int g, int b)
{
	LRgba32 *rgb = (*this)[Idx];
	if (rgb)
	{
		rgb->r = r;
		rgb->g = g;
...
{
	LRgba32 *New = new LRgba32[s];
	if (New)
	{
		memset(New, 0, s * sizeof(LRgba32));
		if (Data)
		{
			memcpy(New, Data, MIN(s, Size)*sizeof(LRgba32));
		}
